...
func CollectProvenance() {
	...
	if onMinikube() {
		print("Running on Minikube!")
		filePath := "/tmp/minikube-sample-audit.log"
		parse(filePath) //using a sample audit log, because
		//currently audit logging is not supported for minikube
	} else {
		filePath := "/tmp/kube-apiserver-audit.log"
...
func (o ObjectLineage) Bisect(argMap map[string]string) string {
	...
	fmt.Printf("Query Attributes Same-parent-relationsips: %v\n", mapRelationships)
	// fmt.Println(specs)
	for _, spec := range specs {
...
func parse(filePath string) {
	...
	if _, err := os.Stat(filePath); os.IsNotExist(err) {
		fmt.Println(fmt.Sprintf("Could not stat the log file %s", err))
		panic(err)
	}
